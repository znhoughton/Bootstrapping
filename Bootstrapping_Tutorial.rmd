...
Now one possible criticism is that the previous method of sampling by-subject samples each subject proportional to how many datapoints that subject contributes, but one last method which surprisingly yields approximately the same results is if we sample by-subject evenly per subject:

```{r}
sampling_by_subject_evenly = function(n_samples) { #sample by-subject evenly
  ...
  for(i in 1:n_samples) {
    ...
    data_sample_factory = data_sample_factory %>% 
      rbind(slice_sample(filter(factory, factory==i), n = length(factory$factory) / length(unique(factory$factory)), replace = TRUE))
...
```{r}
evenly_sampled = sampling_by_subject_evenly(1000)

by_factory_means_evenly = evenly_sampled %>%
  summarize(mean = mean(mean))


grand_mean_evenly = evenly_sampled %>%
  summarize(mean = mean(mean))

entire_dataset_means
by_factory_means_evenly 
grand_mean_entire
grand_mean_evenly

```

Again, though, are the CIs the same?
...
quantile(evenly_sampled$mean, probs = c(0.025, 0.975))
```
